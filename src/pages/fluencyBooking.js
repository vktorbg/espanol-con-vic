...
const FluencyBookingPage = () => {
  const paypalRef = useRef(null);
  const [selectedSlots, setSelectedSlots] = useState([]);
  const [isPayPalLoaded, setIsPayPalLoaded] = useState(false);

  // Fijamos el plan a Fluency Plan
  const selectedPlan = "Fluency Plan";
  ...
  useEffect(() => {
    ...
    // Configuración para el plan Fluency (suscripción)
    script.src =
          ...
          window.paypal.Buttons({
            ...
            createSubscription: (data, actions) => {
              return actions.subscription.create({
                plan_id: "P-0VR89665Y0486153PM7DEGVY", // Fluency plan ID
              });
            },
            onApprove: (data, actions) => {
              alert(data.subscriptionID);
              window.location.href = `/final-landing/?orderID=${data.subscriptionID}&plan=${encodeURIComponent(selectedPlan)}&slots=${encodeURIComponent(selectedSlots.join(", "))}`;
            },
            onError: (err) => {
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        <h1 className="text-4xl font-bold text-primary text-center mb-4">
          Book Your Session - Fluency Plan
        </h1>
...
export default FluencyBookingPage;
